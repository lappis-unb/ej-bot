...
| make rasa-x       | Executa o bot no modo rasa x localmente, que fica disponível em localhost:5002. Sem especificar o domínio, é utilizado o domínio padrão (domain.default.yml). Para especificar outro domínio, por exemplo o boca de lobo, é necessário especificá-lo da seguinte forma: `make rasa-x domain=bocadelobo`.                                                                                                                                                                                                                                                                                                                                                                                                |
...
Para configurar o Rasa X é necessário seguir os seguintes passos:
1. Executar o build (apenas 1 vez) para poder instalar as dependências do rasa x.
2. Executar o comando `make run-actions`. Observação: não é necessário rodar o `make run-api`, pois o Rasa X roda o próprio servidor.
3. Executar o Rasa X através do seguinte comando `make rasa-x`. Se não for especificado o domínio, é utilizado o domínio padrão (domain.default.yml). Para especificar outro domínio, por exemplo o boca de lobo, é necessário especificá-lo da seguinte forma: `make rasa-x domain=bocadelobo`. 
4. Para simular uma conversa do Telegram, passar como primeira palavra de texto o `start` ou `start [id]`. Para simular uma conversa do Webchat, passar como uma primeira mensagem uma url (http://localhost:8001).

![Simular uma conversa passando como primeira mensagem o texto start - Telegram](img/rasax_telegram.png)
![Simular uma conversa passando como primeira mensagem uma url (http://localhost:8001) - Webchat](img/rasax_webchat.png)
