...
class ActionAskVote(Action):
    ...
    def run(
        ...
        custom_logger("Running action_ask_vote")
        ...
        user_can_vote = next(
            (
                event
                for event in can_vote_result
                if event["event"] == "slot" and event["name"] == "user_can_vote"
            ),
            None,
        )
        ...
        try:
            ...
            response += self._dispatch_comment_to_vote(
