...
def test_profile_initialization(mock_profile):
    assert isinstance(mock_profile.user_profile, UserProfile)
...
def test_get_user_profile(mock_profile):
    assert mock_profile.user_profile.user == 1
    assert mock_profile.user_profile.gender == Gender.NOT_FILLED
...
def test_need_to_ask_about_profile(
    mock_profile, conversation_statistics, empty_tracker
):
    should_ask, next_count = mock_profile.need_to_ask_about_profile(
        conversation_statistics, empty_tracker
    )
